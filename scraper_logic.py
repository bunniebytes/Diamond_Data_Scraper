...
High-level flow:
- Open the year menu page and collect yearly links (AL/NL).
...
class Scraper:
    def __init__(
        ...
        *,
        ...
        - **headless**: Run Chrome without a visible UI (useful for servers/CI).
        ...
        self.logger.info("Initializing Chrome driver (headless=%s, profile_dir=%s)", headless, self.profile_dir)
        self.driver = self._build_driver(headless=headless, profile_dir=self.profile_dir)
        self.logger.info("Chrome driver ready")

    def _build_driver(self, *, headless: bool, profile_dir: str) -> webdriver.Chrome:
        """
        ...
        options.add_argument("--disable-gpu")  # Historically helpful on some platforms
    ...
    def scrape(self, *, limit_years: Optional[int] = None) -> None:
        """
        Orchestrate the end-to-end scrape and write CSV outputs to the current directory.

        - **limit_years**: If provided, only scrape the first N yearly links (useful for testing).

        Outputs:
        - `player_hit.csv`, `player_pitch.csv`
        - `team_hit.csv`, `team_pitch.csv`
        - `standing.csv`
        """
        try:
            self.logger.info("Scrape started")
            links = self.get_year_links(YEAR_MENU_URL)
            # Example: subset runs for testing
            ...
            # Always try to close the driver, even if scraping fails mid-run.
            self.close()
        ...
        for filename, df in outputs:
            self.logger.info("Writing %s (%d rows, %d cols)", filename, len(df.index), len(df.columns))
    ...
    def get_year_links(self, menu_url: str) -> List[str]:
        ...
        Important filter:
        - Baseball Almanac also has historical leagues where URLs may end in "a".
          We keep **American League only for years >= 1901** and keep all National League years.
        """
        ...
        for a in anchors:
            ...
            # 'n' => National League (keep all years), 'a' => American League (skip pre-1901).
    ...
    def log_data(self, links: Iterable[str]) -> None:
        ...
        Note: uses a small `sleep()` after navigation; a more robust improvement is to use
        Selenium's WebDriverWait and wait for the tables to exist.
        """
        ...
        for idx, link in enumerate(links_list, start=1):
            ...
            # Events are shared per year (AL/NL pages can both include events; first wins).
    ...
    def get_year_league(self) -> Tuple[Optional[int], Optional[str]]:
        ...
        Returns:
        - `(year, league)` where league is "American League" or "National League"
        - `(None, None)` if the header doesn't match expectations
        """
        try:
        ...
        # Guardrails: American League is considered modern-era starting 1901.
    ...
    def get_data(self) -> Tuple[Dict[str, List[Dict[str, str]]], Dict[str, List[Dict[str, str]]]]:
        ...
        Table names are derived from the table's header text (e.g. "Hitting Statistics").
        """
        ...
        for table in boxed_tables:
            ...
            for row in rows:
                ...
                # Update running table context (name/columns/header-driven dupes).
                if temp_table_name:
                ...
                # Cell-driven rowspan dupes should augment (or replace) the current dupes.
                if temp_dup_from_cells is not None:
                    duplicate_rows = temp_dup_from_cells

                # Only record rows where we have both column headers and a matching-length row.
    ...
    def find_table_name_and_columns(self, row) -> Tuple[Optional[List[str]], Optional[int]]:
        ...
        # NOTE: we normalize stat keys to match `convert_stats_to_df()` expectations.
        # Pages often contain header strings like:
        # "1901 Hitting Statistics League Leaderboards | 1902 →"
        # We want the canonical "Hitting Statistics" key.
        ...
        is_player = bool(re.search(player_pattern, headers[0]))
        if is_player:
    ...
    def find_col_names(self, row) -> Tuple[Optional[List[str]], Optional[Dict[int, List[Any]]]]:
        ...
        - `duplicate_row_val`: mapping from column index -> [value, remaining_rowspan]
          used to fill in missing cells on subsequent rows.
        """
        elements = []
        try:
    ...
    def find_cell_data(
        ...
        Inputs:
        - **num_cols**: expected number of columns for this table (may be None early in parsing)
        - **duplicate_rows**: rolling rowspan state from prior rows
        """
        cells = []
        try:
        ...
        # Only attempt to backfill when we actually know the intended width.
    ...
    def convert_stats_to_df(self, dictionary: Dict[int, Dict[str, Dict[str, List[Dict[str, str]]]]]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        ...
        Each output row is enriched with `Year` and `League`.
        """
    ...
    def convert_events_to_df(self, dictionary: Dict[int, Dict[str, List[str]]]) -> pd.DataFrame:
        ...
        Current `events` shape is:
        - year -> { "Special Events": [...], "Salary": [...] }
        """
...
if __name__ == "__main__":
    ...
    limit = args.limit
    if limit is None:
        # Interactive prompt for convenience when running manually.
        try:
    ...
    Scraper().scrape(limit_years=limit)